...
class DAG(object):
    ...
    def ind_nodes(self, graph=None):
        ...
        return [node for node in graph if node not in dependent_nodes]
...
d = DAG()
d.from_dict({1: [6,9], 2: [6,9], 3: [6,9], 4: [], 5: [6,9], 6: [7,8], 7: [9], 8: [9], 9: [4]})
...
def comp_gain(g, u, rev_part, d):
    gain = 0
    for v in g.predecessors(u):
        if u in rev_part and rev_part[u] == rev_part[v]:
            print("revpart", rev_part[u])
            gain -= cost(g, v, u)
        elif rev_part[v] == d:
            gain += cost(g, v, u)
    for v in g.downstream(u):
        if v in rev_part and rev_part[u] == rev_part[v]:
            gain -= cost(g, v, u)
        elif v in rev_part and rev_part[v] == d:
            gain += cost(g, v, u)
    return gain

def greedy_partition(g, k):
    rev_part = {}
    print("revpart===",rev_part) 
    free = {}
    part = [set() for i in range(k)]
    print("part===",part)
#     for i in range(k):
#         part[i] = []
    ...
    for i in range(k):
        #         s1 = set(g.ind_nodes())
        s = set(n for n in g.graph if free[n] and all(not free[p] for p in g.predecessors(n)))
        print("s-->",s)
        
        
        
        gains = [(u, comp_gain(g, u, rev_part, i)) for u in s]
        print(i,"++++", gains)
        
 
        while len(part[i]) < (0.9 * g.size()/k):
            print("gains:", gains)
            max_gain = max(gains, key=lambda x: x[1])
            print("max_gain:", max_gain)
            gains.remove(max_gain)
            u = max_gain[0]
            print(i, "selecting", u)
            print("ervpart------",rev_part)
            if u in rev_part:
                j = rev_part[u]
                print("j-----",j)
                part[j].remove(u)
            ...
            for v in g.downstream(u):
                ...
                if ready:
                    print(v, "ready")
                    gains.append((v, comp_gain(g, v, rev_part, i)))
    return part
